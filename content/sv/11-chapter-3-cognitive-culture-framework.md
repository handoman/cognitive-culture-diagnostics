...
1. **Serial vs parallel drift**
   - ADHD‑profiler är ofta parallella → fragmenterad men snabb.
   - Systemkulturer kan vara parallella men kräver sekventiell output.
2. **Task switching style**
   - Proaktiv (förutsättande, modellbaserat).
   - Reaktiv (stimulusdriven).
3. **Buffer design (arbetsminne)**
   - Litet men dynamiskt → snabb men bräcklig.
   - Stort men statiskt → minnesstyrka men låg flexibilitet.
4. **Error management**
   - Heuristisk (”good enough”).
   - Deterministisk (”så länge logiken håller är allt okej”).
...
1. **Logikstyrd governance**
   - Beslut baseras på konsistens, inte känsla.
   - Emotioner integreras först när de är semantiskt tolkade.
2. **Affektstyrd governance**
   - Beslut baseras på emotionell valens.
   - Logik används som sekundär modul.
3. **Contextual governance**
   - Beslut styrs av sociala, etiska och relationella variabler.
4. **Efficiency governance**
   - Beslut styrs av optimering: minsta friktion, största outcome.
...
1. **Hur beskriver personen sina minnen?**
   - Som berättelser? → narrativ
   - Som system? → procedural
   - Som regler? → logisk
2. **Hur löser personen konflikter eller känslomässig stress?**
   - Affekt → känsloaktiverad strategi
   - Logik → konsistenskontroll
   - Intuition → koherenssignal
3. **Hur spontant är uttryck av känslor?**
   - Direkt → affektiv
   - Fördröjt → analytisk / intuitiv
   - Kontextdrivet → contextual governance
