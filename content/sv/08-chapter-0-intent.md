## 0.3 Vad läsaren kan förvänta sig (och inte förvänta sig)

### Läsaren kan förvänta sig:

- En ny teoretisk modell: **Systemic Cognitive Culture (SCC)**  
- Ett diagnostiskt tolkningslager: **Meta‑Logical Cognitive Culture (MLCC)**  
- Konkreta kliniska verktyg, inklusive:
  - intervjuguider  
  - differentialdiagnostiska markörer  
  - analysramar  
  - exempelmaterial  
- Jämförelser mellan SCC‑profiler och typiska ADHD‑ och AST‑profiler  
- En monografi som kombinerar:
  - neuropsykologi  
  - fenomenologi  
  - systemteori  
  - spelmetaforer (där det är pedagogiskt motiverat, inte lekfullt i sig)  

Detta är ett professionellt dokument — inte ett självbiografiskt.

Case‑exemplet är närvarande, men aldrig i centrum.  
Det fungerar som en **klinisk dataset‑simulering**, inte som personlig berättelse.

---

### Läsaren bör inte förvänta sig:

- Ett nytt diagnossystem  
- En kritik av DSM som helhet  
- En alternativ ”sanning” om ADHD eller AST  
- Ett universellt verktyg som passar alla  
- En ersättning för klinisk bedömning  

Denna monografi är ett **paradigmkomplement**, inte en paradigmutmanare.

Syftet är inte att ersätta, utan att förklara där nuvarande verktyg blir blinda.

Tänk det som att du inte bygger en ny karta —  
du lägger till höjdkurvor på en befintlig karta.

---

## 0.4 Etiska överväganden

När en modell baseras på både själv‑analys och kliniska observationer uppstår särskilda etiska krav.

### 1. Självrepresentation och transparens

All data som utgår från subjektet är:

- frivilligt tillhandahållen  
- inte normerande  
- inte generaliserande  
- avsedd för teoretisk och metodisk illustration  

Detta är inte ett medicinskt utlåtande och ska inte tolkas som ett sådant.

---

### 2. Ingen rekonstruktion av verkliga patienter

Alla kliniska exempel i monografin är:

- syntetiserade  
- kombinerade  
- anonymiserade  
- designade för att illustrera principer, inte reflektera specifika personer  

Ingen enskild individ kan identifieras, direkt eller indirekt.

---

### 3. Modellen får inte användas för självdiagnostik

MLCC/SCC är ett tolkningsverktyg för kliniker, inte:

- en ersättning för psykiatrisk bedömning  
- ett självskattningsinstrument  
- en diagnostisk algoritm  

Det ska användas i händerna på utbildad personal, inte av privatpersoner.

---

### 4. Fallansvar och klinisk etik

Vid tveksamhet gäller:

Den vanligaste tolkningen ska aldrig ersättas av en mer avancerad teori  
om den inte ger högre precision, lägre risk och bättre patientutfall.

Denna monografi ska aldrig användas som ett argument mot etablerade riktlinjer.  
Den är ett stöd — inte ett undantagstillstånd.

---

## 0.5 Avsiktsförklaring

Detta arbete skrivs inte för att bevisa något om en individ,  
utan för att belysa något om ett mönster.

Om modellen hjälper ens en enda kliniker att:

- förstå en patient som tidigare missförstods  
- undvika en felklassning  
- öppna en ny väg i en utredning  
- ställa en fråga som annars inte hade ställts  

…så har projektet uppfyllt sitt syfte.

Detta är en inbjudan till dialog — inte en dom.  
En början, inte ett slut.
